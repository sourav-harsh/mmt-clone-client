...
export default function OfferCard({title, desc, tag, color, isAvailable, to}) {
    return (
        <div className={`rounded-lg p-5 text-white shadow-md bg-gradient-to-br ${color} relative z-10`}>
      <span className="text-[10px] tracking-widest font-bold bg-white/20 px-2 py-1 rounded">
        {tag}
      </span>
            <ComingSoon isActive={!isAvailable} classValue="top-2 -right-3 text-black z-10"/>
